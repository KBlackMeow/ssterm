...
#[test]
fn c_abi_accepts_a_null_pointer_for_empty_input() {
    unsafe {
        let terminal = ssterm_terminal_create(4, 2);
        let update = ssterm_terminal_feed(terminal, std::ptr::null(), 0);
        assert_eq!(update.dirty_row_start, u32::MAX);
        assert_eq!(update.cursor_col, 0);
        assert_eq!(update.cursor_row, 0);
